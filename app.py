...
# -----------------------------
# Available players and bowlers (only from selected team)
# -----------------------------
...
with tab4:
    ...
    if available_players:
        ...
        **Average Runs per Match:** {avg_runs}  
        ...
        season_runs = merged[(merged["batsman"] == player) & (merged["batting_team"] == team)] \
...
with tab5:
    ...
    if available_bowlers:
        ...
        if st.button("Predict Wickets", key="btn_predict_wickets"):
            bowler_season_data = filtered_deliveries[(filtered_deliveries["bowler"] == bowler) &
